...
import { activateDeliverableTemplateVersionAction, createDeliverableTemplateVersionAction } from "../../../../deliverable-actions";

type VersionSummary = { id: string; version: number; status: string };

export function DeliverableTemplateVersionManager({ templateId, defaultSections }: { templateId: string; defaultSections: string }) {
    ...
    <div className="flex flex-col gap-3">
      ...
      {!isCreating ? (
        ...
        <div className="flex flex-col gap-2 rounded border border-neutral-200 p-3">
          <label htmlFor="new-template-version-sections" className="text-sm font-medium text-neutral-700">
            Sections (JSON)
          </label>
          <textarea id="new-template-version-sections" value={sections} onChange={(e) => setSections(e.target.value)} rows={14} className="rounded border border-neutral-300 px-3 py-2 font-mono text-xs" />
