...
import { activateDocumentThemeVersionAction, createDocumentThemeVersionAction } from "../../../../deliverable-actions";

type VersionSummary = { id: string; version: number; status: string };

export function DocumentThemeVersionManager({ themeId, defaultAccentColor, defaultFontFamily }: { themeId: string; defaultAccentColor: string; defaultFontFamily: string }) {
    ...
    <div className="flex flex-col gap-3">
      {createdVersion ? (
        <div className="flex flex-col gap-2 rounded border border-amber-200 bg-amber-50 p-3">
          <span className="text-sm font-medium text-neutral-900">Nouvelle version v{createdVersion.version} créée ({createdVersion.status})</span>
          <button type="button" disabled={isPending} onClick={() => handleActivate(createdVersion.id)} className="self-start rounded bg-neutral-900 px-3 py-1.5 text-sm font-medium text-white disabled:opacity-50">
      ...
      {!isCreating ? (
        ...
        <div className="flex flex-col gap-2 rounded border border-neutral-200 p-3">
          <label htmlFor="new-theme-version-accent" className="text-sm font-medium text-neutral-700">
            Couleur d&apos;accent
          </label>
          <input id="new-theme-version-accent" type="color" value={accentColor} onChange={(e) => setAccentColor(e.target.value)} className="h-10 w-20 rounded border border-neutral-300" />
          <label htmlFor="new-theme-version-font" className="text-sm font-medium text-neutral-700">
            Police
          </label>
          <input id="new-theme-version-font" value={fontFamily} onChange={(e) => setFontFamily(e.target.value)} className="max-w-md rounded border border-neutral-300 px-3 py-2 text-sm" />
