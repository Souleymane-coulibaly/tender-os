...
import { createRepresentativeAction, type FormActionState } from "../../../../company-profile-actions";
import { REPRESENTATIVE_TYPES, REPRESENTATIVE_TYPE_LABELS, type CompanyRepresentative } from "../../../../../../lib/company-profile-types";
...
export function RepresentativesSection({ clientId, representatives }: { clientId: string; representatives: CompanyRepresentative[] }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <p className="text-sm text-neutral-600">
        Représentants légaux, signataires déclarés et contacts. Le pouvoir de signature réel d&apos;un appel d&apos;offres reste géré dans le dossier administratif de ce Tender.
      </p>
            ...
            <tbody>
              {representatives.map((representative) => (
                <tr key={representative.id} className="border-b border-neutral-100">
                  ...
                  <td className="py-2 pr-4 text-neutral-600">{REPRESENTATIVE_TYPE_LABELS[representative.type]}</td>
                  ...
                  <td className="py-2 pr-4 text-neutral-600">{representative.status}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <form action={formAction} className="flex flex-col gap-3 rounded border border-neutral-200 p-3">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="flex flex-col gap-1">
            ...
            <input id="firstName" name="firstName" required className="rounded border border-neutral-300 px-2 py-1.5 text-sm" />
          </div>
          <div className="flex flex-col gap-1">
            <label htmlFor="lastName" className="text-xs text-neutral-600">
              Nom *
            </label>
            <input id="lastName" name="lastName" required className="rounded border border-neutral-300 px-2 py-1.5 text-sm" />
          </div>
          <div className="flex flex-col gap-1">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="flex flex-col gap-1">
            ...
            <input id="jobTitle" name="jobTitle" className="rounded border border-neutral-300 px-2 py-1.5 text-sm" />
          </div>
          <div className="flex flex-col gap-1">
            <label htmlFor="email" className="text-xs text-neutral-600">
              Email
            </label>
            <input id="email" name="email" type="email" className="rounded border border-neutral-300 px-2 py-1.5 text-sm" />
          </div>
          <div className="flex flex-col gap-1">
            <label htmlFor="phone" className="text-xs text-neutral-600">
              Téléphone
            </label>
            <input id="phone" name="phone" className="rounded border border-neutral-300 px-2 py-1.5 text-sm" />
          </div>
        </div>
        <div className="flex flex-col gap-1">
          ...
          <textarea id="signatureScope" name="signatureScope" rows={2} className="rounded border border-neutral-300 px-2 py-1.5 text-sm" />
        </div>
        {state.error ? (
