...
import { archiveSubcontractorInsuranceAction, createSubcontractorInsuranceAction, type FormActionState } from "../../../subcontractor-actions";
import type { SubcontractorInsurance } from "../../../../../lib/subcontractor-types";

const INITIAL_STATE: FormActionState = {};

export function SubcontractorInsurancesSection({ subcontractorId, insurances }: { subcontractorId: string; insurances: SubcontractorInsurance[] }) {
    ...
    <div className="flex flex-col gap-4">
      ...
      {active.length === 0 ? (
        ...
        <ul className="flex flex-col gap-2 text-sm">
          {active.map((insurance) => (
            <li key={insurance.id} className="flex items-center justify-between rounded border border-neutral-100 px-3 py-2">
              <span>
                <span className="font-medium text-neutral-900">{insurance.type}</span>
                {insurance.insurer ? <span className="text-neutral-600"> — {insurance.insurer}</span> : null}
                {insurance.expiresAt ? <span className="text-neutral-500"> (jusqu&apos;au {new Date(insurance.expiresAt).toLocaleDateString("fr-FR")})</span> : null}
              </span>
              <button type="button" onClick={() => handleArchive(insurance.id)} disabled={archivingId === insurance.id} className="text-red-700 hover:underline disabled:opacity-50">
                Archiver
              </button>
            </li>
          ))}
      ...
      <form action={formAction} className="flex flex-wrap items-end gap-3">
        <div className="flex flex-col gap-1">
          ...
          <input id="type" name="type" required placeholder="ex. RC pro" className="rounded border border-neutral-300 px-2 py-1.5 text-sm" />
        </div>
        <div className="flex flex-col gap-1">
