...
export default async function DeliverableDetailPage({ params }: { params: Promise<{ id: string; deliverableId: string }> }) {
  ...
  if (deliverable.type === "COMPLIANCE_MATRIX") {
    ...
    checklistEntries = await appApiFetch(`/api/v1/deliverables/${deliverableId}/checklist`);
  } else if (deliverable.type === "ANNEXES") {
    // Mission — correctif "aucun moyen de faire avancer le statut d'une annexe" : liste des
    // documents de l'organisation disponibles pour rattachement (sélecteur ci-dessous) — jamais
    // bloquant pour la liste des annexes elle-même si cet appel annexe échoue.
