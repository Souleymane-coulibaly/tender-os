...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string; transactionId: string }> }): Promise<Response> {
  ...
  const backendResponse = await fetch(`${API_BASE_URL}/api/v1/signature-transactions/${transactionId}/evidence`, {
    headers: { Authorization: `Bearer ${token}`, "X-Organization-Id": organizationId },
    cache: "no-store",
  });

  if (!backendResponse.ok || !backendResponse.body) {
    return new Response("Evidence not found.", { status: backendResponse.status });
  }
