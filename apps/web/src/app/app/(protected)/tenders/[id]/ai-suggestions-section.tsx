...
import { applySuggestionAction, fetchTenderSuggestions, mapSuggestionsAction, rejectSuggestionAction, type MapSuggestionsResult } from "../../../ai-suggestion-actions";
import {
 ...
 */
export function AiSuggestionsSection({ tenderId, initialSuggestions, canManage }: { tenderId: string; initialSuggestions: AiSuggestion[]; canManage: boolean }) {
  ...
  async function handleApply(suggestion: AiSuggestion, conflictResolution?: ConflictResolution): Promise<void> {
    setIsPending(true);
    setError(undefined);
  ...
  return (
    <section className="flex flex-col gap-3 rounded border border-neutral-200 p-4 md:col-span-2">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold text-neutral-700">Suggestions IA à valider ({suggestions.length})</h2>
      ...
      <p className="text-xs italic text-neutral-500">
        Chaque suggestion est une proposition issue de l&apos;analyse IA du DCE — aucune n&apos;est jamais appliquée automatiquement. Vous devez
        explicitement l&apos;accepter, la corriger ou la rejeter.
      </p>
      ...
      {suggestions.length === 0 ? (
        ...
        <ul>
          {suggestions.map((suggestion) => (
            <li key={suggestion.id} className="border-b border-neutral-100 py-2 text-sm">
              <div className="flex items-center gap-2">
                ...
                {isCreationSuggestion(suggestion.fieldName) ? (
                  <span className="rounded bg-blue-100 px-1.5 py-0.5 text-xs text-blue-800">Nouvelle entrée</span>
                ) : (
                  <span className="font-medium text-neutral-900">{describeSuggestionField(suggestion.entityType, suggestion.fieldName)}</span>
                )}
                <span className="text-xs text-neutral-500">— confiance {Math.round(suggestion.confidence * 100)}%</span>
              </div>
              <p className="mt-1 text-sm text-neutral-700">{formatSuggestionValue(suggestion.proposedValue)}</p>
              {suggestion.sourceChunkReference ? (
                <p className="mt-1 text-xs italic text-neutral-500">&laquo;&nbsp;{suggestion.sourceChunkReference}&nbsp;&raquo;</p>
              ) : null}
                ...
                conflictIds.has(suggestion.id) ? (
                  <div className="mt-2 flex flex-col gap-2 rounded border border-amber-200 bg-amber-50 p-2">
                    <p className="text-xs text-amber-800">
                      Cette cible porte déjà une valeur — choisissez comment traiter cette suggestion.
                    </p>
