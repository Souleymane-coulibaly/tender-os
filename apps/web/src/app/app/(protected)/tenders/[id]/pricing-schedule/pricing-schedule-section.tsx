...
function DetectedFileCard({
  ...
  async function handleCreate(): Promise<void> {
    setIsPending(true);
    setError(undefined);
    const result = await createPricingScheduleAction(tenderId, { sourceDocumentId: document.documentId });
    setIsPending(false);
  ...
  return (
    <div className="flex items-center justify-between rounded border border-neutral-200 p-3">
      <div>
        ...
        <div className="text-xs text-neutral-500">Détecté dans le DCE — pas encore de chiffrage</div>
        {error ? <p className="mt-1 text-xs text-red-700">{error}</p> : null}
      </div>
      <button type="button" onClick={handleCreate} disabled={isPending} className="rounded bg-neutral-900 px-3 py-1.5 text-xs font-medium text-white disabled:opacity-50">
...
function ControlsPanel({ controls }: { controls: PricingControlsResult | undefined }) {
  ...
  if (controls.findings.length === 0) {
    return <p className="rounded border border-green-200 bg-green-50 p-3 text-xs text-green-800">Aucune anomalie détectée sur cette version.</p>;
  }
  return (
    <div className="rounded border border-neutral-200 p-3">
      <h3 className="text-sm font-semibold">Contrôles</h3>
      <div className="mt-2 flex flex-wrap gap-2 text-xs">
        <span className={`rounded px-2 py-1 ${controlSeverityBadgeClass("ERROR")}`}>Erreurs : {controls.errorCount}</span>
        <span className={`rounded px-2 py-1 ${controlSeverityBadgeClass("WARNING")}`}>Avertissements : {controls.warningCount}</span>
        <span className={`rounded px-2 py-1 ${controlSeverityBadgeClass("INFO")}`}>Infos : {controls.infoCount}</span>
      </div>
      <ul className="mt-3 flex flex-col gap-1 text-xs text-neutral-700">
        {controls.findings.map((finding, index) => (
          <li key={`${finding.code}-${finding.pricingScheduleLineId ?? index}`} className="flex items-start gap-2">
            <span className={`shrink-0 rounded px-1.5 py-0.5 ${controlSeverityBadgeClass(finding.severity)}`}>{finding.severity}</span>
...
function LineRow({
  ...
  if (line.kind !== "PRICE_ITEM") {
    return (
      <tr className="border-t border-neutral-100 bg-neutral-50">
        <td colSpan={6} className="px-3 py-2 text-xs font-medium text-neutral-600">
          {LINE_KIND_LABELS[line.kind]} — {line.designation || <span className="italic text-neutral-400">(sans libellé)</span>}
        </td>
      </tr>
    );
  }
  ...
  async function handleSubmit(event: React.FormEvent): Promise<void> {
    ...
    setIsPending(true);
    setError(undefined);
    const result = await setPricingScheduleLineUnitPriceAction(tenderId, scheduleId, line.id, draftPrice);
    setIsPending(false);
  ...
  return (
    <tr className="border-t border-neutral-100">
      ...
      <td className="px-3 py-2 text-sm text-neutral-600">{line.unit ?? "—"}</td>
      <td className="px-3 py-2 text-sm text-neutral-600">{line.quantity ?? "—"}</td>
      <td className="px-3 py-2">
        {editable ? (
          <form onSubmit={handleSubmit} className="flex items-center gap-1">
            ...
            />
            <button type="submit" disabled={isPending} className="rounded bg-neutral-900 px-2 py-1 text-xs font-medium text-white disabled:opacity-50">
              {isPending ? "…" : "OK"}
            </button>
          </form>
        ) : (
      ...
      <td className="px-3 py-2 text-sm text-neutral-600">{line.proposedTotal ?? "—"}</td>
      <td className="px-3 py-2">
        <span className={`rounded px-2 py-0.5 text-xs ${lineStatusBadgeClass(line.status)}`}>{line.status}</span>
      </td>
    </tr>
  );
}

function ScheduleDetail({ tenderId, detail, onRefresh }: { tenderId: string; detail: DetailState; onRefresh: () => Promise<void> }) {
  ...
  async function handleExtract(): Promise<void> {
    ...
    const result = await extractPricingScheduleVersionAction(tenderId, schedule.id, schedule.sourceDocumentVersionId);
  ...
  async function handleValidate(): Promise<void> {
    ...
    setActionError(undefined);
    const result = await validatePricingScheduleVersionAction(tenderId, schedule.id, currentVersion.id, justification || undefined);
  ...
  async function handleGenerate(): Promise<void> {
    ...
    setActionError(undefined);
    const result = await generatePricingScheduleFinalFileAction(tenderId, schedule.id, currentVersion.id);
  ...
  return (
    <div className="flex flex-col gap-4 rounded border border-neutral-200 p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <h3 className="text-sm font-semibold">{FINANCIAL_DOCUMENT_TYPE_LABELS[schedule.financialDocumentType]}</h3>
          <p className="text-xs text-neutral-500">
            {currentVersion ? `Version ${currentVersion.versionNumber}` : "Aucune extraction pour l'instant"} — {priceableCount > 0 ? `${pricedCount}/${priceableCount} lignes chiffrées` : "—"}
          </p>
        </div>
        <span className={`rounded px-2 py-1 text-xs ${pricingScheduleStatusBadgeClass(schedule.status)}`}>{PRICING_SCHEDULE_STATUS_LABELS[schedule.status]}</span>
      </div>
      ...
      {!currentVersion ? (
        <button type="button" onClick={handleExtract} disabled={isExtracting} className="self-start rounded bg-neutral-900 px-4 py-2 text-sm font-medium text-white disabled:opacity-50">
        ...
        <>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[640px] text-left">
              ...
              <tbody>
                {lines.map((line) => (
                  <LineRow key={line.id} tenderId={tenderId} scheduleId={schedule.id} line={line} editable={!isValidated} onUpdated={() => void onRefresh()} />
                ))}
              </tbody>
            </table>
          </div>
          ...
          <div className="flex flex-wrap items-end gap-3 border-t border-neutral-100 pt-3">
            {!isValidated ? (
              <>
                {controls && controls.errorCount > 0 ? (
                  <div className="flex flex-col gap-1">
                    <label htmlFor={`justification-${schedule.id}`} className="text-xs font-medium text-neutral-600">
              ...
              <>
                <span className="text-xs text-green-700">Version validée — immuable. Une nouvelle extraction créera une nouvelle version pour tout changement de prix.</span>
                ...
                {generatedDocumentId ? (
                  <Link href={`/app/documents/${generatedDocumentId}/download`} className="text-sm font-medium text-blue-700 underline">
...
export function PricingScheduleSection({
  ...
  const undetectedFiles = detectedFinancialDocuments.filter((doc) => !scheduledDocumentIds.has(doc.documentId));
  ...
  async function loadDetail(scheduleId: string): Promise<void> {
    ...
    if (currentVersion) {
      ...
      setDetail({ schedule: result.schedule, versions: result.versions, lines: [], controls: undefined });
    }
      ...
      {undetectedFiles.length > 0 ? (
        <div className="flex flex-col gap-2">
          ...
          {undetectedFiles.map((doc) => (
            <DetectedFileCard key={doc.documentId} tenderId={tenderId} document={doc} onCreated={handleScheduleCreated} />
          ))}
        </div>
      ) : null}

      {schedules.length === 0 ? (
        <p className="rounded border border-neutral-200 p-4 text-sm text-neutral-500">
          Aucun fichier BPU/DPGF/DQE détecté pour l&apos;instant dans le DCE de cet appel d&apos;offres.
        </p>
      ) : (
        <div className="flex flex-col gap-4 lg:flex-row">
          <div className="flex w-full flex-col gap-2 lg:w-64">
            ...
            {schedules.map((schedule) => (
              <button
                ...
                className={`rounded border p-3 text-left text-sm ${selectedScheduleId === schedule.id ? "border-neutral-900 bg-neutral-50" : "border-neutral-200 hover:bg-neutral-50"}`}
              >
                <div className="font-medium">{FINANCIAL_DOCUMENT_TYPE_LABELS[schedule.financialDocumentType]}</div>
                <span className={`mt-1 inline-block rounded px-2 py-0.5 text-xs ${pricingScheduleStatusBadgeClass(schedule.status)}`}>{PRICING_SCHEDULE_STATUS_LABELS[schedule.status]}</span>
              </button>
            ))}
          </div>
          <div className="flex-1">
            {isLoadingDetail ? (
              ...
              <ScheduleDetail tenderId={tenderId} detail={detail} onRefresh={() => loadDetail(detail.schedule.id)} />
            ) : (
              <p className="text-sm text-neutral-500">Sélectionnez un chiffrage pour afficher son détail.</p>
            )}
          </div>
        </div>
      )}
