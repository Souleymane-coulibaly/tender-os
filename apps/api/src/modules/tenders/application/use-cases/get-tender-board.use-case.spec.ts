...
describe("GetTenderBoardUseCase", () => {
  ...
  it("reports the true total count per column, not just the capped items", async () => {
    ...
    const result = await useCase.execute({ organizationId: "org-1", actorRole: "READ_ONLY", limitPerColumn: 2 });
  ...
  it("filters the board by free-text search", async () => {
    const result = await useCase.execute({ organizationId: "org-1", actorRole: "READ_ONLY", search: "mobilier" });
