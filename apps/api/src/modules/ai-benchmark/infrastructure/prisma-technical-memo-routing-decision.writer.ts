...
} from "../../technical-memo/application/ports/routing-decision-writer";

/**
 * Consolidation IA — Checkpoint D. Implémentation du port `RoutingDecisionWriter` DÉFINI PAR
 * `technical-memo` — écrit dans `routing_decisions` (table partagée avec Analyse/Génération/Chat :
 * `technicalMemoSectionId` renseigné, `analysisId`/`generationId`/`conversationId` laissés `null` —
 * voir la contrainte CHECK `routing_decisions_exactly_one_target_check`, migration Checkpoint D).
 * Aucun calcul de coût réel (contrairement à Génération) : Mémoire technique ne possède aucune
 * notion de tarif par appel aujourd'hui, `actualCostAmount`/`currency` restent toujours `null`.
 */
@Injectable()
export class PrismaTechnicalMemoRoutingDecisionWriter implements RoutingDecisionWriter {
  private readonly logger = new Logger(PrismaTechnicalMemoRoutingDecisionWriter.name);
