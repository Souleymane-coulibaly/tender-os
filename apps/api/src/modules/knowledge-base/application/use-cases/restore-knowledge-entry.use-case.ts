...
export type RestoreKnowledgeEntryCommand = Readonly<{
...
@Injectable()
export class RestoreKnowledgeEntryUseCase {
  constructor(
  ...
  async execute(command: RestoreKnowledgeEntryCommand): Promise<KnowledgeEntrySummary> {
    assertHasKnowledgePermission(command.actorRole, KnowledgePermission.Restore);
