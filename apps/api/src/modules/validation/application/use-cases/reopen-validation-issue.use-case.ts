...
export type ReopenValidationIssueCommand = Readonly<{
...
@Injectable()
export class ReopenValidationIssueUseCase {
  constructor(
  ...
  async execute(command: ReopenValidationIssueCommand): Promise<ValidationIssueSummary> {
    ...
    issue.reopen({ resolvedBy: command.actorId, resolutionNote: command.resolutionNote, occurredAt: this.clock.now() });
