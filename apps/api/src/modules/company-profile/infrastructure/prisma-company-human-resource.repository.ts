...
import type { CandidateEntityScope, CandidateScope, ClientScope, CompanyHumanResourceRepository, EntityScope } from "../application/ports/company-satellite.repository";
...
export class PrismaCompanyHumanResourceRepository implements CompanyHumanResourceRepository {
  ...
  async update(scope: EntityScope, patch: Partial<Omit<CompanyHumanResourceRecord, "id" | "organizationId" | "clientAccountId">>): Promise<CompanyHumanResourceRecord | null> {
    const { count } = await this.prisma.companyHumanResource.updateMany({ where: { id: scope.id, organizationId: scope.organizationId, clientAccountId: scope.clientAccountId, candidateCompanyId: null }, data: patch });
  ...
  async findById(scope: EntityScope): Promise<CompanyHumanResourceRecord | null> {
    return this.prisma.companyHumanResource.findFirst({ where: { id: scope.id, organizationId: scope.organizationId, clientAccountId: scope.clientAccountId, candidateCompanyId: null } });
  }
  ...
  async listByCandidate(scope: CandidateScope): Promise<CompanyHumanResourceRecord[]> {
    return this.prisma.companyHumanResource.findMany({
  ...
  async findByIdForCandidate(scope: CandidateEntityScope): Promise<CompanyHumanResourceRecord | null> {
    return this.prisma.companyHumanResource.findFirst({
      where: { id: scope.id, organizationId: scope.organizationId, candidateCompanyId: scope.candidateCompanyId },
    });
  }

  async updateForCandidate(scope: CandidateEntityScope, patch: Partial<Omit<CompanyHumanResourceRecord, "id" | "organizationId" | "clientAccountId" | "candidateCompanyId">>): Promise<CompanyHumanResourceRecord | null> {
    const { count } = await this.prisma.companyHumanResource.updateMany({
      where: { id: scope.id, organizationId: scope.organizationId, candidateCompanyId: scope.candidateCompanyId },
      data: patch,
    });
    if (count === 0) return null;
