...
describe("Connecteurs (connectors) — real HTTP + PostgreSQL (NestJS)", () => {
  ...
  async function registerAndLogin(email: string): Promise<{ userId: string; token: string }> {
    ...
    const registerRes = await fetch(`${baseUrl}/api/v1/auth/register`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email, password, displayName: "Connectors HTTP Test", termsAccepted: true }) });
