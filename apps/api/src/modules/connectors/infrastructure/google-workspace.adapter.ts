...
import { RemoteProviderError, RemoteProviderRateLimitedError, UnsupportedRemoteFileTypeError } from "../domain/errors";
...
async function fetchJson<T>(url: string, schema: z.ZodType<T>, init: RequestInit): Promise<T> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new RemoteProviderError(`Google API request failed (${response.status}): ${body.slice(0, 300)}`);
    }
...
/**
 * Mission §27 — couche Google Workspace unique. `listContainers` retourne "Mon Drive" + les Shared
 * Drives accessibles (mission §28 minimum). `downloadFile` distingue les fichiers Google natifs
 * (Docs/Sheets, export forcé DOCX/XLSX, mission §31) des fichiers binaires classiques (téléchargement
 * direct) — jamais le même chemin de code pour les deux.
 */
@Injectable()
export class GoogleWorkspaceAdapter implements ConnectorProviderAdapter {
  ...
  async fetchAccountInfo(accessToken: string): Promise<OAuthAccountInfo> {
    const info = await fetchJson(USERINFO_URL, UserInfoSchema, { headers: { Authorization: `Bearer ${accessToken}` } });
  ...
  async listContainers(accessToken: string): Promise<readonly RemoteContainer[]> {
    ...
    try {
      ...
      const shared = await fetchJson(`${DRIVE_BASE_URL}/drives`, schema, { headers });
  ...
  async listFolderChildren(accessToken: string, input: { containerId: string; folderId?: string | undefined }): Promise<RemoteFolderListing> {
    ...
    const result = await fetchJson(`${DRIVE_BASE_URL}/files?${params.toString()}`, schema, { headers: { Authorization: `Bearer ${accessToken}` } });
  ...
  async downloadFile(accessToken: string, input: { containerId: string; fileId: string; mimeType: string }): Promise<DownloadedFile> {
    ...
    const meta = await fetchJson(`${DRIVE_BASE_URL}/files/${input.fileId}?fields=id,name,mimeType&supportsAllDrives=true`, metaSchema, { headers: { Authorization: `Bearer ${accessToken}` } });
    ...
    try {
      ...
      if (!response.ok) {
        throw new RemoteProviderError(`Google Drive download failed (${response.status}).`);
      }
      ...
      return { buffer: Buffer.from(arrayBuffer), mimeType: isNative ? exportMimeType! : meta.mimeType, filename: isNative ? `${meta.name}${extension}` : meta.name };
    } finally {
      clearTimeout(timeout);
    }
  }
  ...
  async uploadFile(accessToken: string, input: { containerId: string; folderId: string; filename: string; content: Buffer; mimeType: string }): Promise<RemoteFile> {
    if (input.content.byteLength > MAX_SIMPLE_UPLOAD_BYTES) {
      throw new RemoteProviderError("File exceeds the 20MB simple-upload limit — resumable upload sessions are not implemented this sprint (mission §43, documented limitation).");
    }
    ...
    const created = await fetchJson(`${DRIVE_UPLOAD_URL}?uploadType=multipart&supportsAllDrives=true&fields=id,name,mimeType,size,modifiedTime,webViewLink`, schema, {
      method: "POST",
  ...
  async createCalendarEvent(accessToken: string, input: { title: string; description: string; startAt: Date; endAt: Date; timezone: string }): Promise<{ externalEventId: string }> {
    ...
    const created = await fetchJson(`${CALENDAR_BASE_URL}/calendars/primary/events`, schema, {
      method: "POST",
