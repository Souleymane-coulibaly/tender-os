...
import { RemoteProviderError, RemoteProviderRateLimitedError } from "../domain/errors";
...
/** Correctif audit — l'access token Microsoft (endpoint v2.0) est lui-même un JWT contenant `tid`
 *  (tenant id), pas seulement l'id_token OpenID (qu'on ne demande pas — aucun scope `openid` dans
...
async function fetchJson<T>(url: string, schema: z.ZodType<T>, init: RequestInit): Promise<T> {
  ...
  try {
    ...
    if (response.status === 429) {
      throw new RemoteProviderRateLimitedError();
    }
    if (!response.ok) {
      const body = await response.text().catch(() => "");
      throw new RemoteProviderError(`Microsoft Graph request failed (${response.status}): ${body.slice(0, 300)}`);
    }
...
/**
 * Mission §14 — couche Microsoft Graph unique. `listContainers` retourne le OneDrive de
 ...
 * upload (même API Graph sous-jacente).
 */
@Injectable()
export class MicrosoftGraphAdapter implements ConnectorProviderAdapter {
  ...
  async exchangeCodeForTokens(input: { code: string; codeVerifier: string; redirectUri: string }): Promise<OAuthTokenResult> {
    ...
    const parsed = await fetchJson(TOKEN_URL, TokenResponseSchema, { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body });
  ...
  async fetchAccountInfo(accessToken: string): Promise<OAuthAccountInfo> {
    const me = await fetchJson(`${GRAPH_BASE_URL}/me`, MeResponseSchema, { headers: { Authorization: `Bearer ${accessToken}` } });
  ...
  async listContainers(accessToken: string): Promise<readonly RemoteContainer[]> {
    ...
    try {
      const oneDrive = await fetchJson(`${GRAPH_BASE_URL}/me/drive`, OneDriveSchema, { headers });
    ...
    try {
      const sites = await fetchJson(`${GRAPH_BASE_URL}/sites?search=*`, SitesSchema, { headers });
      for (const site of sites.value) {
        const drivesSchema = z.object({ value: z.array(z.object({ id: z.string(), name: z.string().optional() })) });
        const drives = await fetchJson(`${GRAPH_BASE_URL}/sites/${site.id}/drives`, drivesSchema, { headers });
  ...
  async listFolderChildren(accessToken: string, input: { containerId: string; folderId?: string | undefined }): Promise<RemoteFolderListing> {
    ...
    const result = await fetchJson(url, schema, { headers: { Authorization: `Bearer ${accessToken}` } });
  ...
  async downloadFile(accessToken: string, input: { containerId: string; fileId: string; mimeType: string }): Promise<DownloadedFile> {
    ...
    const meta = await fetchJson(metaUrl, DriveItemSchema, { headers: { Authorization: `Bearer ${accessToken}` } });

    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), REQUEST_TIMEOUT_MS);
    try {
      const response = await fetch(`${GRAPH_BASE_URL}/drives/${input.containerId}/items/${input.fileId}/content`, { headers: { Authorization: `Bearer ${accessToken}` }, signal: controller.signal });
      if (!response.ok) {
        throw new RemoteProviderError(`Microsoft Graph download failed (${response.status}).`);
      }
      const arrayBuffer = await response.arrayBuffer();
      return { buffer: Buffer.from(arrayBuffer), mimeType: meta.file?.mimeType ?? "application/octet-stream", filename: meta.name };
    } finally {
      clearTimeout(timeout);
    }
  }
  ...
  async uploadFile(accessToken: string, input: { containerId: string; folderId: string; filename: string; content: Buffer; mimeType: string }): Promise<RemoteFile> {
    if (input.content.byteLength > MAX_SIMPLE_UPLOAD_BYTES) {
      throw new RemoteProviderError("File exceeds the 20MB simple-upload limit — chunked upload sessions are not implemented this sprint (mission §43, documented limitation).");
    }
    const url = `${GRAPH_BASE_URL}/drives/${input.containerId}/items/${input.folderId}:/${encodeURIComponent(input.filename)}:/content`;
    const item = await fetchJson(url, DriveItemSchema, {
      method: "PUT",
      headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": input.mimeType },
      body: input.content,
    });
    return {
      id: item.id,
  ...
  async createCalendarEvent(accessToken: string, input: { title: string; description: string; startAt: Date; endAt: Date; timezone: string }): Promise<{ externalEventId: string }> {
    ...
    const created = await fetchJson(`${GRAPH_BASE_URL}/me/events`, schema, {
      method: "POST",
