...
describe("Analysis — real HTTP + PostgreSQL (NestJS)", () => {
  ...
  beforeAll(async () => {
    ...
    await prisma.tender.create({
      data: { id: tenderAId, organizationId: orgAId, title: "Tender A — Analysis HTTP", status: "DRAFT", tags: [], createdBy: adminA.userId },
    });
    tenderBId = randomUUID();
    await prisma.tender.create({
      data: { id: tenderBId, organizationId: orgBId, title: "Tender B — Analysis HTTP", status: "DRAFT", tags: [], createdBy: adminB.userId },
    });
