...
import { BUSINESS_ANALYSIS_REPOSITORY, type BusinessAnalysisRepository, type CriterionFindingRecord } from "../ports/business-analysis.repository";
import { executeListTenderFindings, type FindingsPageResult, type ListTenderFindingsQuery } from "./list-tender-findings.shared";

@Injectable()
export class ListTenderCriteriaUseCase {
  constructor(
    private readonly getTenderUseCase: GetTenderUseCase,
    @Inject(BUSINESS_ANALYSIS_REPOSITORY) private readonly businessAnalysisRepository: BusinessAnalysisRepository,
  ) {}

  async execute(query: ListTenderFindingsQuery): Promise<FindingsPageResult<CriterionFindingRecord>> {
    return executeListTenderFindings({
      ...
      query,
      list: (input) => this.businessAnalysisRepository.listCriteria(input),
    });
  }
}
