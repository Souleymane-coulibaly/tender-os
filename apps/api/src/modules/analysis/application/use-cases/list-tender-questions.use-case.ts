...
import { BUSINESS_ANALYSIS_REPOSITORY, type BusinessAnalysisRepository, type QuestionFindingRecord } from "../ports/business-analysis.repository";
import { executeListTenderFindings, type FindingsPageResult, type ListTenderFindingsQuery } from "./list-tender-findings.shared";

@Injectable()
export class ListTenderQuestionsUseCase {
  constructor(
    private readonly getTenderUseCase: GetTenderUseCase,
    @Inject(BUSINESS_ANALYSIS_REPOSITORY) private readonly businessAnalysisRepository: BusinessAnalysisRepository,
  ) {}

  async execute(query: ListTenderFindingsQuery): Promise<FindingsPageResult<QuestionFindingRecord>> {
    return executeListTenderFindings({
      ...
      query,
      list: (input) => this.businessAnalysisRepository.listQuestions(input),
    });
  }
}
