...
export type RejectSignatureRequirementCommand = Readonly<{
...
@Injectable()
export class RejectSignatureRequirementUseCase {
  constructor(
  ...
  async execute(command: RejectSignatureRequirementCommand): Promise<SignatureRequirementSummary> {
    ...
    requirement.reject({ confirmedBy: command.actorId, comment: command.comment, occurredAt: this.clock.now() });
